...
class PoABlock(Block):
    ...
    def mine(self):
        ...
        sk = SigningKey.from_string(pk)
        
        while not self.seal_is_valid():
            header_seal = sk.sign(self.unsealed_header().encode("utf-8"))
            header_seal_int = int.from_bytes(header_seal, byteorder='big')
            self.set_seal_data(header_seal_int)
        return
