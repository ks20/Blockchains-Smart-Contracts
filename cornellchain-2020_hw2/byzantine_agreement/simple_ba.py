...
class SimplePKIBA:
    ...
    def calculate_votes_for(self, round):
        ...
        proposals = list()
        candidates = self.get_proposals_with_threshold(round)
        ...
        for prop in candidates:
            if prop not in self.s_i:
                ...
                signature = util.sign_message(prop, config.SECRET_KEYS[config.node_id])
                self.signatures[prop].append((config.node_id, signature))
    ...
    def get_proposals_with_threshold(self, round):
        ...
        for prop in self.votes.keys():
            if len(self.votes[prop]) >= round:
                if 1 in self.votes[prop]:
